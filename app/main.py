...
from app.core.error_handler import secure_error_handler
...
async def global_exception_handler(request: Request, exc: Exception):
    ...
    return secure_error_handler.handle_exception(request, exc)
...
async def read_discover(request: Request, current_user: Optional[UserInDB] = Depends(try_get_current_user)):
    """Displays the discover page. Requires authentication."""
    if not current_user:
        # If not logged in, redirect to the home page
        return RedirectResponse(url="/")
    return templates.TemplateResponse("discover.html", {"request": request, "user": current_user})
