...
Enhanced with centralized authorization service to fix IDOR vulnerabilities.
"""
import os
import logging
import re
...
async def check_track_access_with_session(track_id: str, current_user: Optional[UserInDB], db: AsyncIOMotorDatabase, client_ip: Optional[str] = None) -> tuple[dict, str]:
    """
    Check if user can access the track using centralized authorization service
    
    Returns:
        (track dict, secure_access_token) if access allowed
        
    Raises:
        HTTPException: If access denied or track not found
    """
    # Use centralized authorization service
    track, access_token = await authorization_service.check_track_access(
        track_id=track_id,
        user=current_user,
        db=db,
        operation="stream",
...
async def get_signed_playlist(
    ...
    # Validate track ID format and check access permissions
    validated_track_id = validate_uuid(track_id, "track_id")
    track, access_token = await check_track_access_with_session(validated_track_id, current_user, db, client_ip)
    ...
    try:
        viewer_id = current_user.id if current_user else "anonymous"
        # Use secure watermark ID generation from the authorization service
        wm_track_id = authorization_service.generate_secure_watermark_id(
            validated_track_id, 
            viewer_id,
            client_ip,
            access_token
...
async def get_segment(
    ...
    try:
        client_ip = request.client.host
        _, access_token = await check_track_access_with_session(base_track_id, current_user, db, client_ip)
