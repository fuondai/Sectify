...
from app.core.file_lock_manager import file_lock_manager
...
ENCRYPTED_STORAGE = "uploads_encrypted"  # Directory to store encrypted files
...
async def process_audio_async(
    ...
    """Background task to process audio asynchronously with lock management"""
    # Acquire processing lock to prevent race conditions
    try:
        async with file_lock_manager.acquire_processing_lock(
            track_id=track_id,
            user_id=user_id,
            operation="audio_processing"
        ) as lock_data:
            logger.info(f"Starting async processing for track {track_id} with lock {lock_data['process_id']}")
            
            # Update progress
            update_progress(track_id, 10, "Starting encryption...", performance_mode)
        
            # For fast mode, skip chaotic encryption for ultra-fast processing
            if performance_mode == 'fast':
                ...
                # Just copy the original file to have an encrypted path 
                ...
                # Initialize chaotic protection for balanced/secure modes
...
async def upload_audio(
    ...
    """
    Upload an audio file and process it asynchronously with enhanced security validation.
    
    Performance Modes:
    - fast: Ultra-fast for testing (0.5s/MB)
    - balanced: Balances speed and security (6s/MB)
    - secure: Maximum security for production (15s/MB)
    """
