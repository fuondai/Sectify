document.addEventListener('DOMContentLoaded', () => {
    ...
    // Function to fetch the list of tracks
    const fetchTracks = async () => {
        try {
            ...
            trackList.innerHTML = ''; // Clear the old list
    ...
    function updateFileRecommendation() {
        ...
        if (fileSizeMB <= 2) {
            if (selectedMode !== 'secure') {
                recommendation = `Small file (${fileSizeMB.toFixed(1)}MB) - Secure Mode is recommended for maximum security.`;
            } else {
                recommendation = `Small file (${fileSizeMB.toFixed(1)}MB) - Good choice for Secure Mode!`;
            }
        } else if (fileSizeMB <= 10) {
            if (selectedMode !== 'balanced') {
                recommendation = `Medium file (${fileSizeMB.toFixed(1)}MB) - Balanced Mode is recommended.`;
            } else {
                recommendation = `Medium file (${fileSizeMB.toFixed(1)}MB) - Optimal choice!`;
            }
        } else {
            if (selectedMode !== 'fast') {
                recommendation = `Large file (${fileSizeMB.toFixed(1)}MB) - Fast Mode is recommended to save time.`;
            } else {
                recommendation = `Large file (${fileSizeMB.toFixed(1)}MB) - Reasonable choice for large files!`;
            }
        }
    ...
    // Handle upload form with progress tracking
