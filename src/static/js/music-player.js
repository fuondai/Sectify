...
class MusicPlayer {
    ...
    loadPlaylist() {
        fetch('/api/audio/list')
            ...
            .then(data => {
                if (data.success) {
                    this.playlist = data.audio_files;
    ...
    playTrack(index) {
        ...
        this.audioElement.src = `/api/audio/stream/${track.id}`;
...
function initLibraryEvents() {
    ...
    if (encryptButton) {
        encryptButton.addEventListener('click', function() {
            ...
            encryptButton.innerHTML = '<i class="fa-solid fa-spinner fa-spin me-1"></i>Đang mã hóa...';
            
            // Gọi API mã hóa
            fetch('/api/audio/encrypt', {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    showAlert(`Lỗi mã hóa: ${data.message}`, 'danger');
                }
            })
            .catch(error => {
                console.error('Error encrypting:', error);
                showAlert('Đã xảy ra lỗi khi mã hóa file.', 'danger');
            })
            .finally(() => {
                encryptButton.disabled = false;
                encryptButton.innerHTML = '<i class="fa-solid fa-lock me-1"></i>Mã hóa bài hát';
            });
        });
    }
